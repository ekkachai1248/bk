 ...
 <div class="card card-default border-0 bg-transparent">
  <div class="card-header align-items-center p-0">
    ...
    <a href="#" class="btn btn-success" data-toggle="modal" data-target="#ModalFormAdd">
      <i class="mdi mdi-plus mr-1"></i> เพิ่มอาหาร
    </a>
  </div>
</div>
...
<div class="card card-default">
  <div class="card-header">
    <h2><i class="mdi mdi-food-fork-drink"></i> อาหาร</h2>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['action'] == "add_food") {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        addLog($conn, $_SESSION['buser_id'], $_SESSION['bfullname'], $_SESSION['brole'], "เพิ่มอาหาร", "เพิ่มอาหาร f_id: {$lastId} {$f_name}");
                ...
                if (!empty($picturesString)) {
                    ...
                    if ($stmtUpdatePictures) {
                        ...
                        if ($stmtUpdatePictures->execute()) {
                            addLog($conn, $_SESSION['buser_id'], $_SESSION['bfullname'], $_SESSION['brole'], "แก้ไขอาหาร", "แก้ไขข้อมูลอาหาร f_id: {$f_id} {$f_name}");
...
$(document).ready(function () {
  ...
  $('#deleteModal').on('show.bs.modal', function (event) {
    ...
    const foodId = button.data('id') || null;
    const foodName = button.data('name') || 'ไม่ระบุ';

    $('#foodName').text(foodName);

    // ใช้ one() เพื่อป้องกัน event ซ้ำซ้อน
...
function deleteFood(foodId) {
  if (!foodId) {
    toastr.error('ไม่พบรหัสอาหารที่ต้องการลบ');
    return;
  }
  ...
  $.ajax({
    ...
    type: 'POST',
    data: { food_id: foodId },
    dataType: 'json', // ป้องกันปัญหาการแปลง JSON
    success: function (response) {
      try {
        const data = response ? response : {}; // ป้องกัน JSON ว่าง
        if (data.success) {
          ...
          const row = $(`a[data-id="${foodId}"]`).closest('tr');
          if (row.length > 0) {
          ...
          toastr.warning(data.message || 'เกิดข้อผิดพลาด');
        }
      } catch (e) {
        console.error('JSON Error:', e);
        toastr.error('เกิดข้อผิดพลาดในการแปลงข้อมูลเซิร์ฟเวอร์');
      }
    },
    error: function (xhr) {
...
<script>
    ...
    $(".dataTables_length select").css({
        ...
        // ใช้ fetch เพื่อดึงข้อมูลจาก fetch_data_foods.php
        fetch('fetch_data_foods.php')
...
$(document).ready(function () {
    ...
    $(document).on('click', '.food-update', function () {
        ...
        // ตั้งค่าการส่งข้อมูล
        $('#updateFoodForm').attr('action', `foods.php?fid=${foodId}`); // foods.php?fid=${foodId}
    });
