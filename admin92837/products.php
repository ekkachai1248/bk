...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST['action'] == "add_product") {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        addLog($conn, $_SESSION['buser_id'], $_SESSION['bfullname'], $_SESSION['brole'], "เพิ่มสินค้า", "เพิ่มสินค้า id: {$lastId} {$p_name}");
                ...
                if (!empty($picturesString)) {
                    ...
                    if ($stmtUpdatePictures) {
                        ...
                        if ($stmtUpdatePictures->execute()) {
                            addLog($conn, $_SESSION['buser_id'], $_SESSION['bfullname'], $_SESSION['brole'], "แก้ไขสินค้า", "แก้ไขข้อมูลสินค้า id: {$p_id} {$p_name}");
