...
<?php include("header.php"); ?>
  ...
  <nav aria-label="breadcrumb" class="my-0 m-0">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">จัดการตารางงานไกด์  </li>
    </ol>
  </nav>
...
<div class="card card-default">
  <div class="card-header">
    <h2><i class="mdi mdi-table"></i> จัดการตารางงานไกด์</h2>
    ...
    <table id="toursTable" class="table table-hover" style="width:100%" cellpadding="20">
      <thead>
        <tr>
          ...
          <th class="bg-success">เลือกไกด์</th>
          <th>วันที่จอง</th>
          <th>รอบ</th>
          <th>จำนวน (คน)</th>
...
?>
        <tr style="font-size: 13px">
          <td>
              ...
              <a href="../images/tours/<?php echo $img[0];?>" target="_blank"><img src="../images/tours/<?php echo $img[0];?>" width="90"></a>
          </td>
          <td><a href="../tour_detail.php?tid=<?php echo $row['item_id'];?>" target="_blank" style="text-decoration: none;" class="text-success"><?php echo $row['item_name'];?></a></td>
            <td>
<?php
// แสดงไกด์ที่ `role` = 'guide' AND `available` = 'active'
$sqlG = "SELECT guide_list FROM tours WHERE tour_id='{$row['item_id']}'";
$rsG = $conn->query($sqlG);
$rowG = $rsG->fetch_assoc();

if (!empty($rowG['guide_list'])) {
    // แปลงค่า guide_list จาก "1;3;4;5" เป็น ['1', '3', '4', '5']
    $guideIDs = explode(';', $rowG['guide_list']);
    
    // แปลงเป็น string สำหรับ SQL IN ('1','3','4','5')
    $guideIDsStr = "'" . implode("','", $guideIDs) . "'";

    // คิวรีดึงข้อมูลไกด์จาก users (เพิ่ม user_id = 1 AND role = 'admin' เสมอ)
    $sqlUsers = "SELECT user_id, fullname FROM users 
                 WHERE (role = 'guide' AND available = 'active' AND user_id IN ($guideIDsStr)) 
                 OR (user_id = 1 AND role = 'admin')
                 ORDER BY user_id ASC";
    
    $rsUsers = $conn->query($sqlUsers);

    // วนลูปแสดงผล
    echo "<select class='form-control' style='font-size:12px' onchange='updateOwner(this, {$row['id']})'>";
    while ($rowUser = $rsUsers->fetch_assoc()) {
        $selected = ($row['owner'] == $rowUser['user_id']) ? "selected" : "";
        echo "<option value='{$rowUser['user_id']}' {$selected}>{$rowUser['fullname']}</option>";
    }
    echo "</select>";
}
?>
...
<!-- แอดมินเลือกไกด์ -->
<script>
function updateOwner(selectElement, bookingId) {
    //console.log("เลือกไกด์ ID:", selectElement.value); // เช็คค่าเลือกจาก select
    //console.log("Booking ID:", bookingId); // เช็คค่า bookingId
    
    $.post('update_guide_booking.php', { owner: selectElement.value, id: bookingId })
        .done(function() {
            swal('', 'เลือกไกด์สำเร็จ', 'success').then(function() {
                // ทำเพิ่มเติมหลังจากการแจ้งเตือน
            });
        })
        .fail(function() {
