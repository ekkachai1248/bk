...
<?php include("header.php"); ?>
  ...
  <nav aria-label="breadcrumb" class="my-0 m-0">
    <ol class="breadcrumb">
      <li class="breadcrumb-item">  <a href="home_seller.php">หน้าหลัก  </a>  </li>
    ...
    <div class="justify-content-center">
        <div class="card card-default card-profile">
          <div class="card-body mt-1 justify-content-center text-center">
            <h4><i class="mdi mdi-cash-multiple"></i> ยอดขายรวมทั้งหมด <span id="total_income" class="text-success"></span> บาท</h4>            
          </div>
        </div>
    </div> 
</div>              
...
<div class="card card-default">
  <div class="card-header">
    <h2><i class="mdi mdi-table"></i> รายการสั่งซื้อ</h2>
    ...
    <table id="toursTable" class="table table-hover" style="width:100%" cellpadding="20">
      <thead>
        <tr>
          <th>...</th>
          <th>ชื่อสินค้า</th>
          <th style="text-decoration: none;">Order ID</th>
          <th>วันที่ซื้อ</th>
          <th>จำนวน</th>
          <th>ชื่อลูกค้า</th>
          <th>สถานะ</th>
...
INNER JOIN products ON booking_details.item_id = products.p_id
WHERE
booking_details.type = 'product' AND
products.s_id = '{$_SESSION['buser_id']}'
ORDER BY
bookings.id DESC
";
...
while ($row = $result->fetch_assoc()){   
    ...
    $sqlImg = "SELECT `p_pictures` FROM `products` WHERE `p_id`='{$row['item_id']}' ";
    $rsImg = $conn->query($sqlImg);
    $rowImg = $rsImg->fetch_assoc();
    $img = explode(";", $rowImg['p_pictures']);
?>
        <tr>
          <td>
              ...
              <a href="../images/products/<?php echo $img[0];?>" target="_blank"><img src="../images/products/<?php echo $img[0];?>" width="90"></a>
          </td>
          <td><a href="../product_detail.php?pid=<?php echo $row['item_id'];?>" target="_blank" style="text-decoration: none;" class="text-success"><?php echo $row['item_name'];?></a></td>
          <td><?php echo $row['id'];?></td>
          <td><?php echo $row['booking_date'];?></td>
          <td><?php echo $row['quantity'];?></td>
          <td style="font-size: 13px"><?php echo $row['fullname'];?><br><?php echo $row['address'];?><br>โทร.<?php echo $row['phone'];?></td>
